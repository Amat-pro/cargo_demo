...
pub struct InboundHttpRoute {
    ...
    pub rule: Vec<InboundHttpRouteRule>,
    pub authorizations: HashMap<AuthorizationRef, ClientAuthorization>,

    // This is required for ordering returned `HttpRoute`s by their creation timestamp
...
impl PartialEq for HeaderMatch {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (Self::Exact(k0, v0), Self::Exact(k1, v1)) => k0 == k1 && v0 == v1,
            (Self::Regex(k0, v0), Self::Regex(k1, v1)) => k0 == k1 && v0.as_str() == v1.as_str(),
            _ => false,
        }
    }
}
